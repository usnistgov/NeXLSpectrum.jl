...
function compress(hss::HyperSpectrum)
    ...
    maxi = mapreduce(ci -> findlast(c -> c != 0.0, hss.counts[:, ci]), max, CartesianIndices(hss))
    data = maxi < depth(hss) ? hss.counts[1:maxi, axes(hss.counts)[2:end]...] : hss.counts
...
"""
function maxpixel(hss::HyperSpectrum, cis::CartesianIndices)
    res = zeros(eltype(hss.counts), depth(hss))
    for ci in cis, i in filter(i -> hss.counts[i, ci] > res[i], eachindex(res))
        res[i] = hss.counts[i, ci]
    end
...
function indexofmaxpixel(hss::HyperSpectrum, cis::CartesianIndices)
    res, cix = zeros(eltype(hss.counts), depth(hss)), CartesianIndex[hss.index[1] for _ = 1:depth(hss)]
    for ci in cis, i in filter(i -> hss.counts[i, ci] > res[i], eachindex(res))
        res[i] = hss.counts[i, ci]
        cix[i] = ci
    end

...
NeXLCore.name(escs::AbstractVector{EscapeArtifact}) =  "Esc[$(join(map(esc->"$(esc.xray)", escs),","))]"
...
charFeature(elm::Element, tr::Transition...; minweight = 1.0e-3, maxE = 1.0e6)::AbstractVector{SpectrumFeature} =
    characteristic(elm, tr, minweight, maxE)

escapeFeature(elm::Element, trs::Transition...; minweight = 0.1, maxE = 1.0e6, escape = n"Si K-L3")::AbstractVector{SpectrumFeature} =
        map(tr -> EscapeArtifact(tr, escape), characteristic(elm, trs, minweight, maxE))

comptonFeature(elm::Element, trs::Transition...; minweight = 1.0e-3)::AbstractVector{SpectrumFeature} =

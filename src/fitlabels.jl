...
Base.show(io::IO, refLab::ReferenceLabel) = print(io::IO, "$(refLab.spectrum[:Name])[$(refLab.roi)]")

_hashrl(spec,roi,xrays) = xor(xor(hash(spec), hash(roi)), hash(xrays))

Base.isequal(c1::ReferenceLabel, c2::ReferenceLabel) =
    (hash(c1) == hash(c2)) && isequal(c1.roi, c2.roi) && isequal(c1.xrays, c2.xrays) && isequal(c1.spectrum, c2.spectrum)
...
Base.show(io::IO, refLab::CharXRayLabel) = print(io::IO, "k[$(name(refLab.xrays)),$(get(refLab.spectrum,:Composition,"Unspecified"))]")
Base.isequal(rl1::CharXRayLabel, rl2::CharXRayLabel) =
    isequal(rl1.roi, rl2.roi) && isequal(rl1.xrays, rl2.xrays) && isequal(rl1.spectrum, rl2.spectrum)
...
Base.isequal(el1::EscapeLabel, el2::EscapeLabel) =
    isequal(el1.roi, el2.roi) && isequal(el1.xrays, el2.xrays) && isequal(el1.spectrum, el2.spectrum)
...
Base.show(io::IO, unk::HyperSpectrumLabel) = print(io, unk.hyperspectrum[:Name]*"[$(unk.index.I)]")
Base.isequal(ul1::HyperSpectrumLabel, ul2::HyperSpectrumLabel) = (ul1.hyperspectrum===ul2.hyperspec) && isequal(ul1.index,ul2.index)

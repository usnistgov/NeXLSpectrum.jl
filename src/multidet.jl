...
"""
function loadmultispec(path::AbstractString, basefn::AbstractString, indexes::UnitRange{Int}=0:3, fnmapper::String = "{1}[{2}].msa")
...
function checkmultispec(specs::AbstractArray{<:Spectrum})
    @assert all(get(sp, :BeamEnergy, 0.0)==get(specs[1], :BeamEnergy, 0.0) for sp in specs[2:end])  "All the beam energies must match."
    @assert all(get(sp, :RealTime, 0.0)==get(specs[1], :RealTime, 0.0) for sp in specs[2:end])  "All the real times must match."
    @assert all(get(sp, :ProbeCurrent, 0.0)==get(specs[1], :ProbeCurrent, 0.0) for sp in specs[2:end])  "All the probe currents must match."
end
...
function binnedspecratio(specs::AbstractArray{<:Spectrum}; minE=100.0, deltaE=100.0)
    sr = specratio(specs)
    map(i->map(ee->mean(sr[i][channel(ee,specs[i]):channel(ee+deltaE,specs[i])]),minE:deltaE:specs[i][:BeamEnergy]/1.8),eachindex(specs))
end

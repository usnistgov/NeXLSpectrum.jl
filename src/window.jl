...
function AP33Tabulation()
    data = CSV.read("AP3_3_mod.csv", skipto=3)
    inter = LinearInterpolation(data[:,1], data[:,2])
    extra = AP33Model()
    return TabulatedWindow("Moxtek AP3.3 tabulation", inter, extra, 1.01582935271)
end


function AP5Tabulation()
    data = CSV.read("AP5.csv")
    inter = LinearInterpolation(data[:,1], data[:,2])
    extra = AP33Model()
    return TabulatedWindow("Moxtek AP5 tabulation",inter, extra, 1.022430368591)
end
""

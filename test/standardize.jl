...
@testset "K2496" begin
    ...
    @test isapprox( value(fs_stds.kratios, kl[bal]), 0.821539, atol=1.0e-5) 
    @test isapprox( σ(fs_stds.kratios, kl[bal]), 0.00142848, atol=1.0e-5) 
    tik = findfirst(l->n"Ti K-L3" in l.xrays, kl)
    @test isapprox( value(fs_stds.kratios, kl[tik]), 0.022283, atol=1.0e-5) 
    ...
    @test isapprox(value(q[n"Ti"]), 0.023047, atol=1.0e-5)
    ...
    @test isapprox( value(fs_stds2.kratios, kl[bal]), 0.821539, atol=1.0e-5) 
    @test isapprox( σ(fs_stds2.kratios, kl[bal]), 0.00142848, atol=1.0e-5) 
    @test isapprox( value(fs_stds2.kratios, kl[tik]), 0.022283, atol=1.0e-5) 

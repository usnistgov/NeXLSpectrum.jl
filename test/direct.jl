...
@testset "DirectFit" begin
    ...
    @test isapprox(mean(df[:, "k[Si K-L3 + 3 others, SiO2]"]), 0.353637, atol=1.0e-6)
    @test isapprox(mean(df[:, "k[Ca K-L3 + 3 others, CaF2]"]), 0.191800, atol=1.0e-6)
    @test isapprox(mean(df[:, "k[Fe K-L3 + 1 other, Fe]"]), 0.065682, atol=1.0e-6)
    @test isapprox(mean(df[:, "k[Fe K-M3 + 3 others, Fe]"]), 0.064266, atol=1.0e-6)

    qs = quantify.(dffrs)
    qdf = asa(DataFrame, qs)
    @test isapprox(mean(qdf[:, "O"]), 0.434627, atol=1.0e-6)
    @test isapprox(mean(qdf[:, "Mg"]), 0.115139, atol=1.0e-6)
    @test isapprox(mean(qdf[:, "Al"]), 0.0489210, atol=1.0e-6)
    @test isapprox(mean(qdf[:, "Si"]), 0.209034, atol=1.0e-6)
    @test isapprox(mean(qdf[:, "Ca"]), 0.108651, atol=1.0e-6)
    @test isapprox(mean(qdf[:, "Fe"]), 0.0791483, atol=1.0e-6)
